'------------------------------------------------------------------------------
' <автоматически создаваемое>
'     Этот код создан программой.
'
'     Изменения в этом файле могут привести к неправильной работе и будут потеряны в случае
'     повторной генерации кода. 
' </автоматически создаваемое>
'------------------------------------------------------------------------------

Option Strict On
Option Explicit On


Partial Public Class PropertyData

    '''<summary>
    '''PropertyDataForm элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents PropertyDataForm As Global.DevExpress.Web.ASPxFormLayout

    '''<summary>
    '''FileManager элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents FileManager As Global.DevExpress.Web.ASPxFileManager

    '''<summary>
    '''DescriptionMemo элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents DescriptionMemo As Global.DevExpress.Web.ASPxMemo

    '''<summary>
    '''YandexFeedCheckBox элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents YandexFeedCheckBox As Global.DevExpress.Web.ASPxCheckBox

    '''<summary>
    '''CianFeedCheckBox элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents CianFeedCheckBox As Global.DevExpress.Web.ASPxCheckBox

    '''<summary>
    '''CreatedDE элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents CreatedDE As Global.DevExpress.Web.ASPxDateEdit

    '''<summary>
    '''AgentNameTB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents AgentNameTB As Global.DevExpress.Web.ASPxTextBox

    '''<summary>
    '''AgentPhoneTB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents AgentPhoneTB As Global.DevExpress.Web.ASPxTextBox

    '''<summary>
    '''AgentEmailTB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents AgentEmailTB As Global.DevExpress.Web.ASPxTextBox

    '''<summary>
    '''AdStatusCB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents AdStatusCB As Global.DevExpress.Web.ASPxComboBox

    '''<summary>
    '''NameMemo элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents NameMemo As Global.DevExpress.Web.ASPxMemo

    '''<summary>
    '''TypeCB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents TypeCB As Global.DevExpress.Web.ASPxComboBox

    '''<summary>
    '''DistrictCB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents DistrictCB As Global.DevExpress.Web.ASPxComboBox

    '''<summary>
    '''ComplexTB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents ComplexTB As Global.DevExpress.Web.ASPxTextBox

    '''<summary>
    '''PriceSpin элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents PriceSpin As Global.DevExpress.Web.ASPxSpinEdit

    '''<summary>
    '''ApartmentAreaSpin элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents ApartmentAreaSpin As Global.DevExpress.Web.ASPxSpinEdit

    '''<summary>
    '''StatusCB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents StatusCB As Global.DevExpress.Web.ASPxComboBox

    '''<summary>
    '''ConditionCB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents ConditionCB As Global.DevExpress.Web.ASPxComboBox

    '''<summary>
    '''RegistrationCB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents RegistrationCB As Global.DevExpress.Web.ASPxComboBox

    '''<summary>
    '''StoveCB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents StoveCB As Global.DevExpress.Web.ASPxComboBox

    '''<summary>
    '''WindowViewCB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents WindowViewCB As Global.DevExpress.Web.ASPxComboBox

    '''<summary>
    '''ToSeaSpin элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents ToSeaSpin As Global.DevExpress.Web.ASPxSpinEdit

    '''<summary>
    '''RoomsSpin элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents RoomsSpin As Global.DevExpress.Web.ASPxSpinEdit

    '''<summary>
    '''FloorSpin элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents FloorSpin As Global.DevExpress.Web.ASPxSpinEdit

    '''<summary>
    '''TotalFloorSpin элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents TotalFloorSpin As Global.DevExpress.Web.ASPxSpinEdit

    '''<summary>
    '''LandAreaSpin элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents LandAreaSpin As Global.DevExpress.Web.ASPxSpinEdit

    '''<summary>
    '''CountryTB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents CountryTB As Global.DevExpress.Web.ASPxTextBox

    '''<summary>
    '''RegionTB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents RegionTB As Global.DevExpress.Web.ASPxTextBox

    '''<summary>
    '''RegionDistrictTB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents RegionDistrictTB As Global.DevExpress.Web.ASPxTextBox

    '''<summary>
    '''LocalityNameTB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents LocalityNameTB As Global.DevExpress.Web.ASPxTextBox

    '''<summary>
    '''StreetNameTB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents StreetNameTB As Global.DevExpress.Web.ASPxTextBox

    '''<summary>
    '''HouseTB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents HouseTB As Global.DevExpress.Web.ASPxTextBox

    '''<summary>
    '''ApartmentTB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents ApartmentTB As Global.DevExpress.Web.ASPxTextBox

    '''<summary>
    '''ActualUntilDE элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents ActualUntilDE As Global.DevExpress.Web.ASPxDateEdit

    '''<summary>
    '''PosrednikCB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents PosrednikCB As Global.DevExpress.Web.ASPxComboBox

    '''<summary>
    '''ComissionTB элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents ComissionTB As Global.DevExpress.Web.ASPxTextBox

    '''<summary>
    '''IpotekaCheckBox элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents IpotekaCheckBox As Global.DevExpress.Web.ASPxCheckBox

    '''<summary>
    '''VIPCheckBox элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents VIPCheckBox As Global.DevExpress.Web.ASPxCheckBox

    '''<summary>
    '''ElitPropertyCheckBox элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents ElitPropertyCheckBox As Global.DevExpress.Web.ASPxCheckBox

    '''<summary>
    '''SaveButton элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents SaveButton As Global.DevExpress.Web.ASPxButton

    '''<summary>
    '''AdStatusDS элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents AdStatusDS As Global.System.Web.UI.WebControls.SqlDataSource

    '''<summary>
    '''PosrednikDS элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents PosrednikDS As Global.System.Web.UI.WebControls.SqlDataSource

    '''<summary>
    '''WindowViewDS элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents WindowViewDS As Global.System.Web.UI.WebControls.SqlDataSource

    '''<summary>
    '''StoveDS элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents StoveDS As Global.System.Web.UI.WebControls.SqlDataSource

    '''<summary>
    '''RegistrationDS элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents RegistrationDS As Global.System.Web.UI.WebControls.SqlDataSource

    '''<summary>
    '''ConditionDS элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents ConditionDS As Global.System.Web.UI.WebControls.SqlDataSource

    '''<summary>
    '''StatusDS элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents StatusDS As Global.System.Web.UI.WebControls.SqlDataSource

    '''<summary>
    '''DistrictDS элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents DistrictDS As Global.System.Web.UI.WebControls.SqlDataSource

    '''<summary>
    '''TypeDS элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents TypeDS As Global.System.Web.UI.WebControls.SqlDataSource

    '''<summary>
    '''PropertyDataDS элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents PropertyDataDS As Global.System.Web.UI.WebControls.SqlDataSource

    '''<summary>
    '''SuccessSavePopup элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents SuccessSavePopup As Global.DevExpress.Web.ASPxPopupControl

    '''<summary>
    '''CBackSave элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents CBackSave As Global.DevExpress.Web.ASPxCallback

    '''<summary>
    '''LoadingPanel элемент управления.
    '''</summary>
    '''<remarks>
    '''Автоматически создаваемое поле.
    '''Для изменения переместите объявление поля из файла конструктора в файл кода программной части.
    '''</remarks>
    Protected WithEvents LoadingPanel As Global.DevExpress.Web.ASPxLoadingPanel
End Class
